      ...
      if (data.transactions && Array.isArray(data.transactions)) {
        ...
        if (confirmedSignatures.length > 0) {
          ...
          const message = failedMints.length > 0
            ? `Successfully minted ${confirmedSignatures.length} of ${mintQuantity} NFTs. Failed: ${failedMints.join(', ')}`
            : `Successfully minted all ${confirmedSignatures.length} NFTs!`;
          
          toast.success(message, {
            action: {
              label: 'View Transaction',
              onClick: () => window.open(getSolanaExplorerUrl(confirmedSignatures[0].signature), '_blank')
            }
          });
          ...
          throw new Error(`All ${mintQuantity} mint transactions failed`);
        }
              ...
              <CardContent className="p-8">
                <div className="grid md:grid-cols-3 gap-8 items-center">
                  ...
                  <div className="text-center md:text-right">
                    <div className="mb-4">
                      <p className="text-sm text-muted-text mb-1">Total Cost</p>
                      <p className="text-2xl font-bold text-hero-text">{totalCostLabel} SOL</p>
                    </div>
                    
                    {connected ? (
                      <Button
                        onClick={handleMint}
                        disabled={isMinting || !collectionStats.isLive || collectionStats.remaining < mintQuantity}
                        className="bg-button-green hover:bg-button-green/90 text-pure-black font-bold px-8 py-3 text-lg min-w-[160px]"
                      >
                        {isMinting ? (
                          <>
                      ...
                      <WalletMultiButton className="bg-button-green hover:bg-button-green/90 text-pure-black font-bold px-8 py-3 text-lg rounded-md" />
                    )}
                    ...
                    {lastMintSignature && (
                      <div className="mt-3">
                        <Button
                          ...
                          className="text-button-green hover:text-button-green/80 text-sm"
                        >
                          View Last Transaction <ExternalLink className="w-4 h-4 ml-1" />
                        </Button>
                      </div>
                    )}
                  </div>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-archivo-black text-hero-text mb-6">
              NFT <span className="text-button-green">Benefits</span>
            </h2>
            <p className="text-lg text-muted-text max-w-2xl mx-auto">
              Unlock exclusive rewards and features with your VapeFi Genesis NFT
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            <Card className="bg-card-bg border-card-border hover:border-button-green/50 transition-colors">
              ...
              <CardContent>
                <ul className="space-y-2 text-muted-text">
                  <li>• 2x puff tracking rewards</li>
            ...
            <Card className="bg-card-bg border-card-border hover:border-button-green/50 transition-colors">
              <CardHeader>
                ...
                <CardTitle className="text-hero-text">Utility Features</CardTitle>
              </CardHeader>
              <CardContent>
                <ul className="space-y-2 text-muted-text">
